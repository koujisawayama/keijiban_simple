...
export function AuthForm() {
  ...
  password,
       ...
       if(profileData && typeof profileData.nickname === 'string' ){
         ...
         if(checkError){
          ...
          if(nickNameCheckData){
              const nicknameDuplicate = nickNameCheckData.some(profile => profile.nickname === profileData.nickname &&  profile.id != loginData.user.id )

              if (nicknameDuplicate) {
                  startShakeAnimation(); // 重複しているのでシェイク！
                  setError('Nickname is already used. You cant sign in with this.');
                  await supabase.auth.signOut();  //念のため、ログアウトも仕掛けておきましょう
                  return;
              }
          }
    ...
    return (
        <div
      ref={shakeRef}
     className={`w-full max-w-md p-8 bg-white rounded-lg shadow-lg ${showErrorAnimation ? 'animate-shake' : ''}`} // ここを追加 シェイク用
      style={{
        ...
        />
          {!isLogin && (
            ...
            type="text"
                label="Nickname"
            value={nickname}
           onChange={handleNicknameChange} //onChangeの処理
           disabled={isLoading}
           required
       />
       )}
